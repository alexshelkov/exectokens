...
fn init(args: InitArgs) {
    STATE.with(|state| {
        ...
        state.supply_cap = args.supply_cap;
    });
}

#[query(name = "supply_cap")]
fn supply_cap() -> Option<u64> {
    STATE.with(|state| state.borrow().supply_cap)
}
...
fn mint(
    ...
    let total_suply = STATE.with(|state| state.borrow().nfts.len() as u64);
    let supply_cap = STATE.with(|state| state.borrow().supply_cap);

    if let Some(supply) = supply_cap {
        if supply > total_suply {
            return Err(ConstrainedError::SupplyCapReached);
        }
    }
...
fn get_metadata(token_id: u64) -> Result<Vec<MetadataPart>> {
    // ic_cdk::setup();
...
#[query(name = "logo")]
fn logo() -> Option<LogoResult> {
    STATE.with(|state| state.borrow().logo.to_owned())
}
